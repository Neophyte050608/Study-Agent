...
后端采用 domain-first / package-by-feature。详细包结构、模块 ownership、Knowledge/RAG 细分和迁移规则，以 `PACKAGE_CONVENTIONS.md` 为唯一细则来源；本节只保留 AI agent 开发时必须立即遵守的硬规则。

修改或新增后端代码前：

1. 先阅读 `PACKAGE_CONVENTIONS.md`，按业务域选择包位置。
2. 涉及架构边界时同步阅读 `ARCHITECTURE.md` 和 `docs/development/backend-guidelines.md`。
3. 若变更了包结构、模块职责或边界规则，优先更新 `PACKAGE_CONVENTIONS.md`，再按需更新本文件中的入口说明。

硬性规则：

- 不要新增业务代码到旧式顶层 `service`、`entity`、`mapper`、`dto` 包。
- Controller 只调用 application service，不直接调用 MyBatis Mapper、外部 SDK、Agent runtime 或 tool executor。
- MyBatis DO 和 Mapper 放在 `<domain>.infrastructure.persistence`。
- Controller-facing DTO 放在 `<domain>.api`；application 内部 DTO 放在 `<domain>.application.dto`。
- domain 层不能依赖 Spring Web、Controller DTO、MyBatis DO、外部 SDK 或 infrastructure adapter。
- 不要继续把新职责塞进 `RAGService`；Knowledge/RAG 新逻辑按 `PACKAGE_CONVENTIONS.md` 的细分包放置。
- 跨模块 SSE/stream 传输支撑放在 `common.stream`；业务流式编排留在所属业务域。
- 不要为了“架构完整”创建空 port、空 adapter、空 facade 或空目录。

后续如果新增稳定业务域或调整模块归属，只维护 `PACKAGE_CONVENTIONS.md` 的详细清单，避免本文件和包规范出现两份真相源。
