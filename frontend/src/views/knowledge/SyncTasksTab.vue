<template>
  <div class="space-y-6">
    ...
    <div>
      <h4 class="text-sm font-bold text-content mb-4 flex items-center gap-2">
         <span class="material-symbols-outlined text-slate-400">receipt_long</span>
         同步日志详情
      </h4>
      <div class="bg-surface-raised border border-slate-100 rounded-xl shadow-sm overflow-hidden">
         <div v-for="(item, idx) in reports" :key="idx" class="p-4 border-b border-slate-50 flex items-start gap-4 hover:bg-surface-field transition-colors last:border-0">
           <div class="mt-1.5 w-2 h-2 rounded-full flex-shrink-0" :class="item.status === 'success' ? 'bg-emerald-500' : 'bg-red-500'"></div>
           <div class="flex-1 min-w-0">
             <h5 class="text-sm font-bold text-content mb-1 truncate">{{ item.fileName }}</h5>
             <p class="text-xs text-content-secondary leading-relaxed">{{ item.message }}</p>
           </div>
