<template>
  <div class="space-y-6">
    <div class="bg-surface-raised rounded-xl p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row items-start gap-6">
      ...
      <div class="flex-1">
        <h3 class="text-lg font-bold text-content mb-2">默认本地知识库</h3>
        <p class="text-sm text-content-secondary mb-4">包含所有通过本地目录同步与上传的文档数据。</p>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            <span class="text-slate-400 block mb-1 font-medium">同步路径配置</span>
            <div class="font-mono text-xs bg-surface-field p-3 rounded-lg border border-slate-100 whitespace-pre-wrap text-content-secondary min-h-[40px]">
              {{ config.paths || '未配置' }}
            </div>
          </div>
          <div>
            <span class="text-slate-400 block mb-1 font-medium">忽略目录</span>
            <div class="font-mono text-xs bg-surface-field p-3 rounded-lg border border-slate-100 text-content-secondary min-h-[40px]">
              {{ config.ignoreDirs || '无' }}
            </div>
          </div>
        </div>
    ...
    <div class="bg-surface-raised rounded-xl p-6 shadow-sm border border-slate-100">
      <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-5">
        <div class="flex-1">
          <h3 class="text-lg font-bold text-content mb-2">本地图谱索引</h3>
          <p class="text-sm text-content-secondary mb-4">基于同步目录中的 Markdown 笔记生成 `Local Graph` 检索索引，并可直接激活到当前运行实例。</p>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
            ...
            <div>
              ...
              <div class="text-sm text-content-secondary bg-surface-field p-3 rounded-lg border border-slate-100 min-h-[40px]">
                {{ indexStatus.nodeCount || 0 }}
              </div>
            </div>
            <div>
              <span class="text-slate-400 block mb-1 font-medium">Ollama 模型</span>
              <div class="font-mono text-xs bg-surface-field p-3 rounded-lg border border-slate-100 min-h-[40px] text-content-secondary">
        ...
        <div class="flex flex-col items-stretch gap-3 w-full md:w-auto md:min-w-[220px]">
          ...
          <button
            ...
            class="bg-brand py-2.5 px-5 rounded-lg text-white font-bold flex items-center justify-center gap-2 hover:bg-brand-hover transition-all active:scale-95 shadow-sm disabled:opacity-60"
          >
