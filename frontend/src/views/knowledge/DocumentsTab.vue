  ...
  <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
      <div class="relative">
        ...
        <input v-model="searchQuery" @keyup.enter="fetchDocuments" type="text" placeholder="搜索文档名称或状态..." class="pl-9 pr-4 py-2 text-sm border border-slate-200 bg-white dark:bg-slate-900 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 w-64 outline-none transition-all" />
      </div>
      <div class="flex items-center gap-4">
        <button @click="fetchDocuments" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">刷新</button>
        <div class="text-sm font-medium text-slate-500 dark:text-slate-400">共 <span class="text-indigo-600">{{ total }}</span> 份文档记录</div>
      </div>
    </div>
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="(item, idx) in documents" :key="item.docId" class="hover:bg-slate-50/50 transition-colors">
            <td class="px-6 py-4 font-medium text-slate-900 dark:text-slate-100 max-w-[200px] truncate" :title="item.docName">{{ item.docName }}</td>
            <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ item.sourceType }}</td>
            <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ item.chunkCount }}</td>
            <td class="px-6 py-4">
              <button @click="toggleEnabled(item)" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold transition-colors" :class="item.enabled ? 'bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-100' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 hover:bg-slate-200'">
    ...
    <div class="p-4 border-t border-slate-100 flex justify-between items-center bg-slate-50/50 text-sm">
      <button @click="pageNo > 1 && (pageNo--, fetchDocuments())" :disabled="pageNo <= 1" class="px-3 py-1 bg-white dark:bg-slate-900 border border-slate-200 rounded text-slate-600 dark:text-slate-400 disabled:opacity-50 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">上一页</button>
      <div class="flex items-center gap-2 text-slate-500 dark:text-slate-400">
        <span>第</span>
        <select v-model="pageNo" @change="fetchDocuments" class="bg-white dark:bg-slate-900 border border-slate-200 rounded pl-3 pr-8 py-1 text-slate-700 dark:text-slate-300 outline-none focus:ring-1 focus:ring-indigo-500 appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%236b7280%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22M6%208l4%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[right_0.25rem_center] bg-no-repeat">
      ...
      <button @click="documents.length === pageSize && (pageNo++, fetchDocuments())" :disabled="documents.length < pageSize" class="px-3 py-1 bg-white dark:bg-slate-900 border border-slate-200 rounded text-slate-600 dark:text-slate-400 disabled:opacity-50 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">下一页</button>
    </div>
  </div>
</template>
<script setup>
