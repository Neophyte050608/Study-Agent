  ...
  <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
      <div class="flex items-center gap-3">
        <button @click="$emit('back')" class="text-slate-500 hover:text-slate-800 transition-colors flex items-center">
          <span class="material-symbols-outlined text-[20px]">arrow_back</span>
        </button>
        <h3 class="font-bold text-slate-800">文档分块明细 <span class="text-sm font-normal text-slate-500 ml-2" v-if="docId">{{ docId }}</span></h3>
      </div>
      <div class="text-sm font-medium text-slate-500">共 <span class="text-indigo-600">{{ total }}</span> 个分块</div>
    </div>

    <div v-if="!docId" class="flex flex-col items-center justify-center py-24 bg-slate-50/50">
      <div class="w-16 h-16 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-400 mb-5">
        <span class="material-symbols-outlined text-3xl">ads_click</span>
      </div>
      <h3 class="text-lg font-bold text-slate-900 mb-2">未选择文档</h3>
      <p class="text-sm text-slate-500 mb-6">请先在文档列表中点击“详情”以查看具体分块内容。</p>
    </div>
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="(item, idx) in chunks" :key="item.chunkId" class="hover:bg-slate-50/50 transition-colors">
            <td class="px-6 py-4 font-mono text-slate-500">#{{ item.childIndex }}</td>
            <td class="px-6 py-4 text-slate-700">
              <div class="line-clamp-3 text-xs leading-relaxed">{{ item.snippet }}</div>
            </td>
            <td class="px-6 py-4">
              <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold" :class="item.enabled ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-slate-100 text-slate-500 border border-slate-200'">
    ...
    <div v-if="docId" class="p-4 border-t border-slate-100 flex justify-between items-center bg-slate-50/50 text-sm">
      <button @click="pageNo > 1 && (pageNo--, fetchChunks())" :disabled="pageNo <= 1" class="px-3 py-1 bg-white border border-slate-200 rounded text-slate-600 disabled:opacity-50 hover:bg-slate-50 transition-colors">上一页</button>
      <div class="flex items-center gap-2 text-slate-500">
        <span>第</span>
        <select v-model="pageNo" @change="fetchChunks" class="bg-white border border-slate-200 rounded pl-3 pr-8 py-1 text-slate-700 outline-none focus:ring-1 focus:ring-indigo-500 appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%236b7280%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22M6%208l4%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[right_0.25rem_center] bg-no-repeat">
      ...
      <button @click="chunks.length === pageSize && (pageNo++, fetchChunks())" :disabled="chunks.length < pageSize" class="px-3 py-1 bg-white border border-slate-200 rounded text-slate-600 disabled:opacity-50 hover:bg-slate-50 transition-colors">下一页</button>
    </div>
  </div>
</template>
<script setup>
