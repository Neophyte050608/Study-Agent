<template>
  <div class="bg-surface text-content min-h-screen relative">
    ...
    <div class="flex items-center gap-4">
      <h1 class="text-xl font-bold tracking-tight text-indigo-700">意图列表 <span class="text-content-secondary dark:text-slate-500 font-medium text-sm ml-2">/ 筛选、分页与批量下线意图节点</span></h1>
    </div>
    <div class="flex items-center gap-2">
      <RouterLink to="/intent-tree" class="px-3 py-1.5 text-xs rounded border border-slate-200 text-content-secondary dark:text-slate-500 hover:bg-surface-field">
        回到树页
      </RouterLink>
      <button @click="reload" :disabled="loading" class="px-3 py-1.5 text-xs rounded border border-slate-200 text-content-secondary dark:text-slate-500 hover:bg-surface-field disabled:opacity-50">
        刷新
      </button>
    </div>
  </header>
  ...
  <main class="min-h-screen bg-surface-field pt-24 pb-10 px-8 space-y-6 transition-all duration-300" :class="sidebarCollapsed ? 'ml-20' : 'ml-64'">
    <section class="rounded-xl border border-slate-200 bg-surface-raised p-4">
      <div class="flex flex-wrap items-center gap-2">
        <div class="relative w-full md:w-[420px]">
          ...
          <span class="material-symbols-outlined absolute left-2 top-[10px] text-content-muted text-base">search</span>
        </div>
    ...
    <section class="bg-surface-raised rounded shadow-sm p-4">
      <div v-if="selectedIndexes.length > 0" class="flex items-center justify-between border-y border-slate-200 bg-surface-field px-4 py-2 -mx-4 mb-3">
        <span class="text-sm text-content-secondary">已选 {{ selectedIndexes.length }} 项</span>
        <div class="flex items-center gap-2">
          <button class="h-8 rounded border text-xs px-3" @click="runBatchOffline" :disabled="loading">批量下线</button>
        </div>
      </div>

      <div v-if="loading" class="py-10 text-center text-content-secondary dark:text-slate-500">加载中...</div>
      <div v-else-if="pageRows.length === 0" class="py-10 text-center text-content-secondary dark:text-slate-500">
          ...
          <tr v-for="row in pageRows" :key="row.index" class="border-t">
            ...
            <td>
              <div class="space-y-0.5">
                <div class="flex items-center gap-2">
                  <span class="font-semibold text-content">{{ row.name || '-' }}</span>
                  <span class="rounded-full border border-slate-200 bg-surface-field px-2 py-0.5 font-mono text-xs text-content-secondary dark:text-slate-500">
                    {{ row.intentId }}
                  </span>
                </div>
              </div>
            ...
            <td>
              <span :class="row.enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-content-secondary dark:text-slate-500'" class="px-2 py-0.5 rounded text-[12px]">
                {{ row.enabled ? '启用' : '下线' }}
              </span>
            </td>
            <td>
      ...
      <div v-if="showPagination" class="mt-4 flex flex-wrap items-center justify-between gap-2 text-sm text-content-secondary dark:text-slate-500">
