  ...
  <div class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased min-h-screen">
    ...
    <header class="fixed top-0 right-0 h-16 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-100 flex justify-between items-center px-8 z-40 transition-all duration-300" :class="sidebarCollapsed ? 'left-20' : 'left-64'">
      <div class="flex items-center gap-4">
        <h1 class="text-xl font-bold tracking-tight text-indigo-700 dark:text-indigo-400">模型配置 <span class="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium text-sm ml-2">/ 针对不同业务场景配置专属的 AI 智能代理</span></h1>
      </div>
    ...
    <main class="pt-24 p-8 min-h-screen bg-slate-50 dark:bg-slate-800/50 transition-all duration-300" :class="sidebarCollapsed ? 'ml-20' : 'ml-64'">
      <div class="max-w-7xl mx-auto">
        <header class="mb-10">
          <h3 class="text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">动态 Agent 设置</h3>
          <p class="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-2 max-w-2xl">针对不同业务场景配置专属的 AI 智能代理，优化响应逻辑、模型深度与生成策略。</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <template v-for="(config, key) in configs" :key="key">
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm overflow-hidden transition-all duration-300 border border-slate-200 hover:border-indigo-300"
                 :class="key === 'DecisionLayerAgent' ? 'lg:col-span-4 flex flex-col h-full' : (key === 'EvaluationAgent' ? 'lg:col-span-8' : 'lg:col-span-12')">
              
              <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-white dark:bg-slate-900">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <h4 class="font-bold text-lg text-slate-900 dark:text-slate-100">{{ key }}</h4>
                    <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500" v-if="key === 'EvaluationAgent'">核心职责：深度分析候选人回答质量及多维度打分。</p>
                    <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500" v-if="key === 'DecisionLayerAgent'">核心职责：管理逻辑流与分支路由。</p>
                    <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500" v-if="key === 'LearningProfileAgent'">核心职责：通过数据洞察生成个性化成长路径与学习建议。</p>
                  </div>
                </div>
              ...
              <div :class="key === 'DecisionLayerAgent' ? 'p-6 flex-1 space-y-6' : 'p-8'">
                <div :class="key === 'DecisionLayerAgent' ? 'space-y-4' : (key === 'EvaluationAgent' ? 'grid grid-cols-1 md:grid-cols-2 gap-8' : 'grid grid-cols-1 md:grid-cols-4 gap-8')">
                  ...
                  <div class="space-y-2">
                    <label class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 dark:text-slate-500">Provider 服务商</label>
                    <select class="w-full bg-slate-50 dark:bg-slate-800/50 border-none rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 transition-all outline-none" v-model="config.provider">
                  ...
                  <div class="space-y-2">
                    <label class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 dark:text-slate-500">Model Name 模型名称</label>
                    <input class="w-full bg-slate-50 dark:bg-slate-800/50 border-none rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 transition-all outline-none" type="text" v-model="config.modelName" />
                  </div>
                  
                  <div class="space-y-2" :class="key === 'EvaluationAgent' ? 'md:col-span-2' : ''">
                    <label class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 dark:text-slate-500">API Key 密钥</label>
                    <input class="w-full bg-slate-50 dark:bg-slate-800/50 border-none rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 transition-all outline-none" placeholder="留空则使用系统全局默认 Key" type="password" v-model="config.apiKey" />
                  </div>
                  
                  <div class="space-y-4" :class="key === 'EvaluationAgent' ? 'md:col-span-2' : ''">
                    <div class="flex justify-between items-center">
                      <label class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 dark:text-slate-500">Temperature 采样温度</label>
