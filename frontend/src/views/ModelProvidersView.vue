<template>
  <div class="bg-surface text-content antialiased min-h-screen">
    <header
      class="fixed top-0 right-0 h-16 bg-surface-overlay/80 backdrop-blur-xl border-b border-divider-light flex justify-between items-center px-8 z-40 transition-all duration-300"
      :class="sidebarCollapsed ? 'left-20' : 'left-64'"
    >
      <div class="flex items-center gap-4">
        <h1 class="text-xl font-bold tracking-tight text-brand-text">
          模型提供商管理
          <span class="text-content-secondary font-medium text-sm ml-2">/ 多模型候补与健康监控</span>
        </h1>
      </div>
      <button
        class="bg-brand text-white px-5 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 hover:bg-brand-hover transition-all active:scale-[0.98] disabled:opacity-60"
    ...
    <main
      class="pt-24 p-8 min-h-screen bg-surface transition-all duration-300"
      :class="sidebarCollapsed ? 'ml-20' : 'ml-64'"
    >
      <div class="max-w-7xl mx-auto space-y-6">
        <section class="grid grid-cols-1 xl:grid-cols-5 gap-4">
          <div class="bg-surface-raised rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5">
            <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">候选模型数</p>
            <p class="mt-3 text-3xl font-bold text-content">{{ candidates.length }}</p>
            <p class="mt-1 text-sm text-content-secondary">当前已配置的模型候选总数</p>
          </div>
          <div class="bg-surface-raised rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5">
            <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">默认模型</p>
            <p class="mt-3 text-lg font-bold text-content break-all">{{ stats?.defaultModel || '-' }}</p>
            <p class="mt-1 text-sm text-content-secondary">通用路由默认优先模型</p>
          </div>
          <div class="bg-surface-raised rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5">
            <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">深度推理模型</p>
            <p class="mt-3 text-lg font-bold text-content break-all">{{ stats?.deepThinkingModel || '-' }}</p>
            <p class="mt-1 text-sm text-content-secondary">用于 THINKING 路由的首选模型</p>
          </div>
          <div class="bg-surface-raised rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5">
            <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">召回模型</p>
            <p class="mt-3 text-lg font-bold text-content break-all">{{ stats?.retrievalModel || '-' }}</p>
            <p class="mt-1 text-sm text-content-secondary">用于 RETRIEVAL 路由的首选模型</p>
          </div>
          <div class="bg-surface-raised rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5">
            <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">回退次数</p>
            <p class="mt-3 text-3xl font-bold text-content">{{ stats?.runtime?.routeFallbackCount ?? 0 }}</p>
            <p class="mt-1 text-sm text-content-secondary">运行时累计路由回退计数</p>
          </div>
        </section>

        <p v-if="hint" class="text-sm font-medium" :class="hintTypeClass">{{ hint }}</p>

        <div v-if="loading && candidates.length === 0" class="bg-surface-raised rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-10 text-center text-content-secondary">
          <span class="material-symbols-outlined animate-spin text-4xl">autorenew</span>
          <p class="mt-4">正在加载模型配置...</p>
        </div>

        <div v-else-if="sortedCandidates.length === 0" class="bg-surface-raised rounded-xl shadow-sm border border-dashed border-slate-300 dark:border-slate-700 p-10 text-center">
          <span class="material-symbols-outlined text-5xl text-slate-300 dark:text-slate-600">hub</span>
          <p class="mt-4 text-lg font-semibold text-content">尚未配置模型候选</p>
          <p class="mt-2 text-sm text-content-secondary">创建第一个模型提供商后，即可在这里查看健康状态与运行时统计。</p>
        </div>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <article
            ...
            :key="candidate.id"
            class="bg-surface-raised rounded-xl shadow-sm border transition-all hover:border-indigo-300 dark:hover:border-indigo-500 overflow-hidden"
            :class="getCardBorderClass(candidate.name)"
          >
            <div class="p-6 border-b border-divider-light flex items-start justify-between gap-4">
              <div class="min-w-0">
                <div class="flex items-center gap-3 flex-wrap">
                  <span class="w-2.5 h-2.5 rounded-full shrink-0" :class="getCircuitDotClass(candidate.name)"></span>
                  <h2 class="text-lg font-bold text-content truncate">{{ candidate.displayName || candidate.name }}</h2>
                ...
                <p class="mt-2 text-sm text-content-secondary break-all">{{ candidate.name }}</p>
              </div>
              ...
              <label class="relative inline-flex items-center cursor-pointer shrink-0">
                ...
                />
                <div class="w-11 h-6 bg-slate-200 dark:bg-slate-700 rounded-full peer-focus:ring-2 peer-focus:ring-indigo-500 transition-all peer-checked:bg-brand"></div>
            ...
            <div class="p-6 space-y-6">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">提供商</p>
                  <p class="mt-1 text-sm font-medium text-content">{{ candidate.provider || '-' }}</p>
                </div>
                <div>
                  <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">模型</p>
                  <p class="mt-1 text-sm font-medium text-content break-all">{{ candidate.model || '-' }}</p>
                </div>
                <div class="sm:col-span-2">
                  <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">地址</p>
                  <p class="mt-1 text-sm font-medium text-content break-all">{{ candidate.baseUrl || '-' }}</p>
                </div>
                <div class="sm:col-span-2">
                  <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">密钥</p>
                  <div class="mt-1 flex items-center gap-3 flex-wrap">
                    <span class="text-sm font-medium text-content">{{ candidate.apiKeyMasked || '未配置' }}</span>
                    <button
                      class="bg-surface-hover text-content-secondary px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-all disabled:opacity-60"
                ...
                <div>
                  <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">优先级</p>
                  <p class="mt-1 text-sm font-medium text-content">{{ candidate.priority ?? 100 }}</p>
                </div>
                <div>
                  <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">能力标记</p>
              ...
              <section class="rounded-xl bg-surface-field border border-slate-200 dark:border-slate-800 p-4">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-xs font-bold uppercase tracking-wider text-content-secondary">运行时统计</p>
                    <p class="mt-1 text-sm text-content-secondary">展示当前熔断器与请求结果汇总。</p>
                  </div>
                  <span class="text-xs px-2 py-0.5 rounded-full font-medium" :class="getCircuitBadgeClass(candidate.name)">
                ...
                <div class="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div class="rounded-lg bg-surface-raised border border-divider p-3">
                    <p class="text-xs text-content-secondary">请求</p>
                    <p class="mt-1 text-lg font-bold text-content">{{ getDetail(candidate.name)?.requestCount ?? 0 }}</p>
                  </div>
                  <div class="rounded-lg bg-surface-raised border border-divider p-3">
                    <p class="text-xs text-content-secondary">成功</p>
                    <p class="mt-1 text-lg font-bold text-emerald-600 dark:text-emerald-400">{{ getDetail(candidate.name)?.successCount ?? 0 }}</p>
                  </div>
                  <div class="rounded-lg bg-surface-raised border border-divider p-3">
                    <p class="text-xs text-content-secondary">失败</p>
                    <p class="mt-1 text-lg font-bold text-red-600 dark:text-red-400">{{ getDetail(candidate.name)?.failureCount ?? 0 }}</p>
                  </div>
                  <div class="rounded-lg bg-surface-raised border border-divider p-3">
                    <p class="text-xs text-content-secondary">连续失败</p>
            ...
            <footer class="px-6 py-4 bg-slate-50/70 dark:bg-slate-800/50 border-t border-divider-light">
              <div class="flex flex-col gap-3">
                <div class="flex flex-wrap items-center gap-3">
                  <button
                    class="bg-brand text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-brand-hover transition-all disabled:opacity-60 inline-flex items-center gap-2"
                  ...
                  <button
                    class="bg-surface-hover text-content-secondary px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 bg-slate-950/55 backdrop-blur-sm flex items-center justify-center p-4">
      <div class="w-full max-w-3xl bg-surface-raised rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div class="px-6 py-4 border-b border-divider-light bg-surface-field flex items-center justify-between">
          <div>
            <h3 class="text-lg font-bold text-content">{{ editingId === null ? '新增模型候选' : '编辑模型候选' }}</h3>
            <p class="text-sm text-content-secondary mt-1">
              {{ editingId === null ? '创建新的模型候补配置并纳入路由管理。' : '保留唯一标识不变，更新模型参数与优先级。' }}
            </p>
          </div>
          <button class="text-content-muted hover:text-slate-700 dark:hover:text-slate-300 transition-colors" @click="closeModal">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5 max-h-[75vh] overflow-y-auto">
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">唯一标识</label>
            <input
              v-model.trim="form.name"
              :disabled="editingId !== null"
              :class="editingId !== null ? 'bg-surface-hover text-content-secondary cursor-not-allowed' : inputClass"
          ...
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">显示名称</label>
          ...
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">提供商</label>
          ...
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">模型标识</label>
          ...
          <div class="space-y-2 md:col-span-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">API 地址</label>
          ...
          <div class="space-y-2 md:col-span-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">API Key</label>
            <div class="relative">
              <input
          ...
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">优先级</label>
          ...
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase tracking-wider text-content-secondary">路由类型</label>
          ...
          <label class="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-surface-field p-4 cursor-pointer">
            <input v-model="form.isPrimary" class="w-4 h-4 accent-indigo-600" type="checkbox" />
            <div>
              <p class="text-sm font-semibold text-content">主模型</p>
              <p class="text-xs text-content-secondary">启用主动健康监控，并作为优先级策略的重要候选。</p>
            </div>
          </label>
          ...
          <label class="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-surface-field p-4 cursor-pointer">
            ...
            <div>
              <p class="text-sm font-semibold text-content">支持深度推理</p>
              <p class="text-xs text-content-secondary">允许该模型参与 THINKING 路由。</p>
            </div>
          </label>

          <label class="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-surface-field p-4 cursor-pointer md:col-span-2">
            <input v-model="form.enabled" class="w-4 h-4 accent-indigo-600" type="checkbox" />
            <div>
              <p class="text-sm font-semibold text-content">启用候选</p>
              <p class="text-xs text-content-secondary">关闭后该模型不会参与路由，但配置会保留。</p>
            </div>
          </label>
        </div>

        <div class="px-6 py-4 border-t border-divider-light bg-surface-field flex justify-end gap-3">
          <button class="bg-surface-hover text-content-secondary px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-all" @click="closeModal">
            取消
          </button>
          <button
            class="bg-brand text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-brand-hover transition-all disabled:opacity-60 inline-flex items-center gap-2"
            @click="saveCandidate"
            :disabled="saving"
          >
...
const inputClass = 'bg-surface-field'
