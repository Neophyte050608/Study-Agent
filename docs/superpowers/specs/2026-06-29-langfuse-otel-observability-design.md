...
Langfuse 当前对 Java/Spring Boot 场景更适合通过 OpenTelemetry 导出 trace，而不是让业务代码直接依赖 Langfuse SDK。因此第一版采用 OpenTelemetry OTLP exporter，将内部 AI observation event 映射为 OpenTelemetry span，再由 Langfuse OTLP endpoint 接收。
