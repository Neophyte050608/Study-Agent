...
多值字段统一支持 `List<?>` 和单个字符串两种输入形式，包括 `taskPlan`、`completedSteps`、`observations`、`availableTools`、`disabledTools`、`safetyRules`。source 负责 trim、过滤空项并保持输入顺序。
...
- `ToolTaskContextAttributesTest`：验证文本、列表和空值读取策略。
